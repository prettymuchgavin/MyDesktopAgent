...
class OllamaEngine:
    ...
    def analyze_desktop_task(self, image_b64: str, system_persona: str, goal: str, current_step: str, plan_steps: List[str], mouse_context: Optional[Dict[str, Any]] = None, tool_context: Optional[str] = None) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            "   - Call External MCP Tool: {\"action\": \"mcp_tool\", \"server\": \"server_name\", \"tool\": \"tool_name\", \"arguments\": {...}}\n"
            "   - Add / Install MCP Server: {\"action\": \"add_mcp_server\", \"name\": \"server_name\", \"command\": \"npx\", \"args\": [\"-y\", \"@modelcontextprotocol/server-...\"], \"env\": {\"KEY\": \"val\"}} or with \"url\": \"http://...\"\n"
            "   - Remove MCP Server: {\"action\": \"remove_mcp_server\", \"name\": \"server_name\"}\n\n"
            "   MCP CONFIGURATION & SETUP INSTRUCTIONS:\n"
            "   - If the user asks to add/install an MCP server (or gave an online URL / GitHub / .md file with instructions):\n"
            "     1. Use `fetch_url` or `read_file` to read the server's documentation/requirements.\n"
            "     2. Call `add_mcp_server` to configure, launch, and persist the server into config.yaml immediately.\n"
            "     3. Once connected, the newly discovered tools will be available for you and the user.\n\n"
